      ...
      e = {
        ...
        showProducts: function (s) {
          ...
          let r = t("<form>").attr({
            ...
            }).html("1 ta mahsulot yoki xizmat narxi").append(p),
            u = t("<div>").attr({
              class: "smart-basket__product-price"
            }).html(`Narxi / ${a.smartBasketCurrency}`).append(c).append(d);
          c.click(() => {
            d.fadeToggle()
          }), p.click(t => {
          ...
          let m = t("<div>").attr({
            ...
            }).html(`jami / ${a.smartBasketCurrency}`),
            k = t("<div>").attr({
              class: "smart-basket__product-delete"
            }).text("O'chirish");
            ...
            for (let a in s[n]) {
              if (count == 1) {
                ...
                if (s[n][a] < 100) {
                    ...
                    product +=  '\lb' + s[n][a].toString().replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, "$1 ") + " so'm";
                  }
              }else if(count == 4){
                if (s[n][a] > 100) {
                    product +=  " x " + s[n][a].toString().replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, "$1 ") + " so'm";
                ...
                product +=  " = <b>" + s[n][a].toString().replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, "$1 ") + ' so`m</b>';
              }else{
                product +=  "<b><i>" + s[n][a] + '</i></b>';
              }
              ...
              let hp = s[n][a].toString().replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, "$1 ");
              if ("sbName" === a && (c.append(`<span>${s[n][a]}</span>`),

                  p.val(s[n][a])), "sbSize" === a && void 0 !== s[n][a] && (c.children("span").append(`<span class="smart-basket__product-size"> Размер: ${s[n][a]}</span>`), d.val(s[n][a])),
                  
                  "sbPrice" === a && (_.html(hp + " " + "so'm"), h.val(s[n][a])), "sbPriceCommon" === a) {
                let t = +s[n][a];
                let laz = t.toString().replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, "$1 ");
                y.val(laz + " " + "so'm")
              }
              "sbQuantity" === a && (v.val(s[n][a]), k.val(s[n][a]));
              count++;
            }
            var inputProducts = document.createElement("input");
            inputProducts.type = "hidden";
            inputProducts.name = "products[]";
            product = product.replace(/(\r\n|\n|\r)/gm, "")
            product = product.replace('\lb', "\n")
            inputProducts.value = product.replace(' - ','');
            inputProducts.setAttribute('required','');
            products.appendChild(inputProducts);
            f.click(function (a) {
                a.preventDefault();
                let r = t(this).data("sbId"),
                  ...
                  i = +h.val(),
                  o = i * n;
                n > 1 && (o = i * --n, k.val(n), e.updateBasket(s, r, n, o), e.getSmartBasketMinState(s, "updateSmartBasketMin"), e.commonResult(s, "updateCommonResult")), h.val(i), y.val(o.toString().replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, "$1 ") + ' ' + "so'm"), v.val(n)
              }),
              b.click(function (a) {
                a.preventDefault();
                let r = t(this).data("sbId"),
                  ...
                  i = +h.val(),
                  o = i * n;
                n >= 1 ? (o = i * ++n, k.val(n), e.updateBasket(s, r, n, o), e.getSmartBasketMinState(s, "updateSmartBasketMin"), e.commonResult(s, "updateCommonResult")) : n = 1, h.val(i), y.val(o.toString().replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, "$1 ") + ' ' + "so'm"), v.val(n)
        ...
        stateBasket: function () {
          ...
          return e.getSmartBasketMinState(o), c.click(function () {
            ...
            if (void 0 !== r[n]) {
              let a = t(this).html();
              return t(this).text("savatchada mavjud"), t("body").append(e.alertBlock("alreadyAdded")), setTimeout(() => {
                t(this).html(a)
              }, 1500), !1
            } {
              let i = {};
        ...
        commonResult: function (e, s, r) {
          let n = 0,
            ...
            }).text("Savatcha bo'sh. Siz hech qanday mahsulot qoshmagansiz").fadeOut(),
            l = t("<div>").attr({
          ...
          if (0 == +n ? o.fadeIn() : o.fadeOut(), r) t("." + l.attr("class")).css("display", "block"), setTimeout(() => {
            ...
            if (!s) return d.html(`<span>jami mahsulotlar: </span> ${n}`), p.html(`<span>jami bo'lib: </span> <input name="finalPrice" class="smart-basket__total-cost" type="text" value="${i.toString().replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, "$1 ")} ${a.smartBasketCurrency}">`), c.append(o).append(l).append(d).append(p), c;
            0 == +n && t("." + o.attr("class")).css("display", "block"), t("." + d.attr("class")).html("<span>jami mahsulotlar: </span> "), t("." + d.attr("class")).html(`<span>jami mahsulotlar: </span> ${n}`), t("." + p.attr("class")).html("<span>jami bo'lib: </span> "), t("." + p.attr("class")).html(`<span>jami bo'lib: </span> <input name="finalPrice" class="smart-basket__total-cost" type="text" value="${i.toString().replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, "$1 ")} so'm">`);
            let hammasiBolibde = document.querySelector('#hammasiBolibde');
            let total_price = document.querySelector('#total_price');
            hammasiBolibde.innerHTML = i.toString().replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, "$1 ") + " " + "so'm";
            total_price.value = i.toString().replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, "$1 ") + " " + "so'm";

            let savatlardegide = document.querySelector('#savatlardegide');
            savatlardegide.innerHTML = n + " " + "ta";
          }
        },
        ...
        userForm: function () {
          ...
          let m = t("<a>").attr({
              class: "SmartSotvoladiBtn",
              href: "buyurtmaKorzina.html"
            }).text("sotib olish"),
            k = t("<div>").attr({
              class: "smart-basket__footer"
            });
          r.append(n).append(o).append(c), a.agreement.isChecked && r.append(d), k.append(u).append(m), s.append(r).append(k);
